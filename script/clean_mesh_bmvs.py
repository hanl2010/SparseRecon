...
def clean_points_by_mask(points, scan, data_dir, imgs_idx=None, minimal_vis=0, mask_dilated_size=11):
    cameras = np.load('{}/scan{}/cameras.npz'.format(data_dir, scan))
    ...
    for i in imgs_idx:
        ...
        mask_image = imageio.v3.imread(mask_lis[i])
        assert len(mask_image.shape) == 3 and mask_image.shape[2]==4
        mask_h, mask_w = mask_image.shape[:2]
        mask_image = np.broadcast_to(mask_image[:, :, -1:], (mask_h, mask_w, 3))
        mask_image = cv.resize(mask_image, (image_w, image_h), cv.INTER_NEAREST)
        ...
        mask_image = np.concatenate([np.ones([1, image_w]), mask_image, np.ones([1, image_w])], axis=0)
        mask_image = np.concatenate([np.ones([image_h+2, 1]), mask_image, np.ones([image_h+2, 1])], axis=1)

        in_mask = (pts_image[:, 0] >= 0) * (pts_image[:, 0] <= image_w) * (pts_image[:, 1] >= 0) * (
                pts_image[:, 1] <= image_h) > 0
        curr_mask = mask_image[(pts_image[:, 1].clip(0, image_h+1), pts_image[:, 0].clip(0, image_w+1))]
...
def clean_mesh_faces_by_mask(mesh_file, new_mesh_file, data_dir, scan, imgs_idx, minimal_vis=0, mask_dilated_size=11):
...
def clean_mesh_faces_outside_frustum(old_mesh_file, new_mesh_file, data_dir, imgs_idx, H=1200, W=1600, mask_dilated_size=11,
    ...
    for i in imgs_idx:
        ...
        # mask_file = f"{DTU_DIR}/eval_mask/scan{scan}/mask/{i:08d}.png"
        # mask_image = cv.imread(mask_file)
        mask_image = imageio.v3.imread(mask_lis[i])
        assert len(mask_image.shape) == 3 and mask_image.shape[2] == 4, print(mask_image.shape)
        mask_h, mask_w = mask_image.shape[:2]
        mask_image = np.broadcast_to(mask_image[:, :, -1:], (mask_h, mask_w, 3))
        mask_image = cv.resize(mask_image, (image_w, image_h), cv.INTER_NEAREST)
...
if __name__ == "__main__":
    ...
    clean_mesh_faces_by_mask(old_mesh_file, clean_mesh_file, args.mask_dir, scan, imgs_idx, minimal_vis=1,
