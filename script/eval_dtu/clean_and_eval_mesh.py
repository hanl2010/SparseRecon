...
def clean_points_by_mask(points, scan, data_dir, imgs_idx=None, minimal_vis=0, mask_dilated_size=11):
    cameras = np.load('{}/scan{}/cameras_sphere.npz'.format(data_dir, scan))
    mask_list = sorted(glob('{}/scan{}/mask/*.png'.format(data_dir, scan)))
    ...
    for i in imgs_idx:
        ...
        mask_image = cv.imread(mask_list[i])
...
def clean_mesh_faces_by_mask(mesh_file, new_mesh_file, scan, data_dir, imgs_idx, minimal_vis=0, mask_dilated_size=11):
    old_mesh = trimesh.load(mesh_file)
    old_vertices = old_mesh.vertices[:]
    old_faces = old_mesh.faces[:]
    old_colors = old_mesh.visual.vertex_colors[:]
    old_face_colors = old_mesh.visual.face_colors[:]
    mask = clean_points_by_mask(old_vertices, scan, data_dir, imgs_idx, minimal_vis, mask_dilated_size)
    ...
    new_colors = old_colors[np.where(mask)][:, :3]
    face_colors = old_face_colors[np.where(mask)][:, :3]

    new_mesh = trimesh.Trimesh(new_vertices, new_faces, face_colors=face_colors, vertex_colors=new_colors)
...
def clean_mesh_faces_outside_frustum(old_mesh_file, new_mesh_file, data_dir, imgs_idx, H=1200, W=1600, mask_dilated_size=11,
                                     isolated_face_num=500, keep_largest=False):
    ...
    cameras = np.load('{}/scan{}/cameras_sphere.npz'.format(data_dir, scan))
    mask_lis = sorted(glob('{}/scan{}/mask/*.png'.format(data_dir, scan)))
    ...
    for i in imgs_idx:
        ...
        # mask_file = f"{data_dir}/scan{scan}/mask/{i:03d}.png"
...
if __name__ == "__main__":
    ...
    base_path = os.path.dirname(args.input_mesh)
    mesh_name = os.path.basename(args.input_mesh)
    mesh_base_name = os.path.splitext(mesh_name)[0]
    ...
    old_mesh_file = args.input_mesh
    clean_mesh_file = os.path.join(base_path, "clean_{:03d}_{}.ply".format(scan, mesh_base_name))
    final_mesh_file = os.path.join(base_path, "final_{:03d}_{}.ply".format(scan, mesh_base_name))

    print("cull mesh ....")
    clean_mesh_faces_by_mask(old_mesh_file, clean_mesh_file, scan, args.mask_dir, imgs_idx, minimal_vis=1,
                             mask_dilated_size=mask_kernel_size)

    if int(args.scan_id) in [24, 34, 38, 40, 82, 106, 110, 114, 118]:
        keep_largest = True
        clean_outliers(old_mesh_file=clean_mesh_file, new_mesh_file=clean_mesh_file)
    else:
        keep_largest = False
    # keep_largest = False
    clean_mesh_faces_outside_frustum(clean_mesh_file, final_mesh_file, args.mask_dir, imgs_idx, mask_dilated_size=mask_kernel_size,
                                     keep_largest=False)

    print("eval mesh ....")
    Offical_DTU_Dataset = args.DTU_DIR
    script_dir = os.path.dirname(os.path.abspath(__file__))
    cmd = f"python {script_dir}/eval.py --data {final_mesh_file} --scan {scan} --mode mesh --dataset_dir {Offical_DTU_Dataset} --vis_out_dir {base_path}"
    os.system(cmd)

    print("finish processing scan%d" % args.scan_id)
