...
def cull_scan(scan, mesh_path, result_mesh_file, instance_dir, image_indices, mask_cull=False, ):
    ...
    for image_index in image_indices:
        mask_path = os.path.join(mask_dir, f"{image_index:03d}.png")
    ...
    if mask_cull:
        ...
        for i, image_index in tqdm(enumerate(image_indices),  desc="Culling mesh given masks"):
            pose = pose_all[image_index]
            w2c = torch.inverse(pose).cuda()
            intrinsic = intrinsics_all[image_index].cuda()
...
if __name__ == "__main__":
    ...
    elif args.overlap_type == "large_overlap":
        image_indices = [23, 24, 33]
    else:
        raise NotImplementedError(args.overlap_type)
    ...
    cull_scan(scan, ply_file, result_mesh_file, instance_dir=os.path.join(args.mask_dir, f'scan{args.scan_id}'), image_indices=image_indices)
    print("eval mesh ....")
