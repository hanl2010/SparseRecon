...
if len(args.cases) != 0:
    scenes = [dtu_scene[case] for case in args.cases]
else:
    scenes = dtu_scene
...
for scene in scenes:
    ...
    for i in range(n_images):
        ...
        t = w2c[:3, 3]
        qvec = rotmat2qvec(r)
        sub_images.append([i+1, *qvec, *t, 1, images_list[i]])
    ...
    shutil.rmtree(os.path.join(dense_path, "images"), ignore_errors=True)
    shutil.rmtree(os.path.join(dense_path, "sparse"), ignore_errors=True)
